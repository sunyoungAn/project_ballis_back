...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product>  {
	...
	@Query(value="SELECT "
	        ...
	        + "LEFT JOIN Selling sell ON prod.id = sell.product AND (sell.sellingStatus = 1 OR sell.sellingStatus = 17) "
	        + "WHERE img.pageDiv = 1 AND img.mainImageDiv = 1 "
	        + "AND (sell.wishPrice IS NULL OR "
	        + "sell.wishPrice = ("
	        + "SELECT MIN(wishPrice) "
	        + "FROM Selling "
	        + "WHERE product = prod.id AND (sellingStatus = 1 OR sellingStatus = 17))) "
	        ...
	        + "AND (subSell.sellingStatus = 1 OR subSell.sellingStatus = 17)))) "
	...
	@Query(value="SELECT "
	        ...
	        + "prod.sizeMin, prod.sizeMax, prod.sizeUnit, img.imagePath, bd.brandName, buy.wishPrice, sell.wishPrice, sell.inventoryDiv, con.price) "
	        + "FROM Product prod "
	        + "INNER JOIN Image img ON prod.id = img.targetId "
	        + "INNER JOIN Brand bd ON prod.brand = bd.brandId "
	        + "LEFT JOIN Buying buy ON prod.id = buy.product AND buy.buyingStatus = 1 "
	        + "LEFT JOIN Selling sell ON prod.id = sell.product AND (sell.sellingStatus = 1 OR sell.sellingStatus = 17) "
	        ...
	        + "FROM Buying "
	        + "WHERE product = prod.id AND buyingStatus = 1))) "
	...
	@Query(value="SELECT "
	        ...
	        + "sell.sellingStatus, sell.productSize, MIN(sell.wishPrice), sell.inventoryDiv, sell.member.memberNumber, MIN(sell.id)) "
	        + "FROM Product prod "
	        + "INNER JOIN Image img ON prod.id = img.targetId "
	        + "LEFT JOIN Selling sell ON prod.id = sell.product "
	        + "WHERE prod.id = :productid "
	        + "AND img.pageDiv = 1 AND img.mainImageDiv = 1 AND sell.inventoryDiv = 1 AND sell.sellingStatus = 17 "
	        + "AND sell.wishPrice = "
	        + "(SELECT MIN(sell2.wishPrice) FROM Selling sell2 "
	        + "WHERE sell2.product = prod.id AND sell2.productSize = sell.productSize "
	        + "AND sell2.inventoryDiv = 1 AND sell2.sellingStatus = 17) "	 
	...
	@Query(value="SELECT "
	        ...
	        + "GROUP BY prod.id, prod.productEngName, prod.productKorName, prod.modelNumber, img.imagePath, "
	        + "sell.sellingStatus, sell.productSize, sell.inventoryDiv, sell.member.memberNumber, sell.id "
			+ "ORDER BY MIN(sell.wishPrice) ASC"
	        ,nativeQuery = false)
	...
	@Query(value="SELECT "
            ...
            + "ORDER BY "
            + "CASE :sort "
    		+ "WHEN 1 THEN COUNT(DISTINCT con.id) END DESC, "
    		+ "CASE :sort "
    		+ "WHEN 2 THEN COUNT(DISTINCT wi.id) END DESC, "
		    + "CASE :sort "
		    + "WHEN 3 THEN prod.launchingDate END DESC, "
		    + "CASE :sort "
	...
	@Query(value="SELECT "
	        ...
	        + "(prod.id, prod.productEngName, prod.productKorName, prod.modelNumber, img.imagePath, "
	        + "buy.buyingStatus, buy.productSize, MAX(buy.wishPrice), buy.member.memberNumber, MIN(buy.id)) "
	        + "FROM Product prod "
	        + "INNER JOIN Image img ON prod.id = img.targetId "
	        + "LEFT JOIN Buying buy ON prod.id = buy.product "
	        + "WHERE prod.id = :productid "
